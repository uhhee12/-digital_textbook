...
/**/
	// 뷰어 컨트롤을 위해서 추가함
	var zoomView_val = null;
	function viewport(){
		if( parent.ZOOMVALUE == undefined ){
			parent.ZOOMVALUE = 1;
		};
		zoomView_val = parent.ZOOMVALUE;
	};
 ...
 * Get slider ID value.
 * 
 * @param {Number} node The slider's unique value.
 * @Date      2018. 10. 29
 */

Powerange.prototype.getSliderId = function(node) {
  var id = node.classList[0].split('js-opacity')[1];
...
Powerange.prototype.step = function(sliderSize, handleSize) {
  var dimension = sliderSize - handleSize
    ...
    , interval = percentage.of(part, dimension) * zoomView_val /**/
    , steps = [];
...
Horizontal.prototype.setStart = function(start) {
  var begin = (start === null) ? this.options.min : start
    ...
    , offset = percentage.of(part, (this.slider.offsetWidth - this.handle.offsetWidth) * zoomView_val ) /**/
...
Horizontal.prototype.setPosition = function(val) {
  this.handle.style.left = val/zoomView_val + 'px'; /**/
  this.slider.querySelector('.range-quantity').style.width = val/zoomView_val + 'px'; /**/
};
...
Horizontal.prototype.onmousedown = function(e) {
  ...
  this.handleOffsetX = this.handle.offsetLeft * zoomView_val; /**/
  this.restrictHandleX = (this.slider.offsetWidth - this.handle.offsetWidth) * zoomView_val; /**/
...
Vertical.prototype.setStart = function(start) {
  var begin = (start === null) ? this.options.min : start
    , part = percentage.from(begin - this.options.min, this.options.max - this.options.min) || 0
    , offset = percentage.of(part, (this.slider.offsetHeight - this.handle.offsetHeight) * zoomView_val ) /**/
    , position = (this.options.step) ? closest.find(offset, this.steps) : offset;
...
Vertical.prototype.setPosition = function(val) {
  this.handle.style.bottom = val / zoomView_val + 'px'; /**/
  this.slider.querySelector('.range-quantity').style.height = val / zoomView_val + 'px'; /**/
};
...
Vertical.prototype.onmousedown = function(e) {
  ...
  this.handleOffsetY = (this.slider.offsetHeight - this.handle.offsetHeight - this.handle.offsetTop) * zoomView_val; /**/
  this.restrictHandleY = (this.slider.offsetHeight - this.handle.offsetHeight) * zoomView_val; /**/
  this.unselectable(this.slider, true);
